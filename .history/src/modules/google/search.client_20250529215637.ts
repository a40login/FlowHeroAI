...
import { getGoogleSearchApiKey, getGoogleSearchCseId } from './store-module-google';
 ...
 */
export async function googleSearch(query: string): Promise<GoogleSearchResults> {
  const googleApiKey = getGoogleSearchApiKey();
  const googleCseId = getGoogleSearchCseId();
  ...
  try {
    return await apiAsync.googleSearch.query({ googleApiKey, googleCseId, query });
  } catch (error: any) {
